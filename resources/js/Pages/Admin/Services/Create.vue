...
<template>
    ...
    <AdminLayout title="Crear Nuevo Servicio">
        <div class="max-w-4xl mx-auto bg-white dark:bg-secondary-900 shadow-sm rounded-xl overflow-hidden border border-primary-100 dark:border-secondary-800 p-8">
            <h2 class="text-xl font-bold mb-6 text-secondary-900 dark:text-white">Detalles del Servicio</h2>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <!-- Image -->
                <div>
                     <label class="block font-medium text-secondary-900 dark:text-white mb-2">Imagen de Portada (Privada)</label>
                     <input 
                        type="file" 
                        @input="form.image = $event.target.files[0]"
                        class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100"
                    >
                    <p class="text-xs text-secondary-500 mt-1">La imagen se guardará en almacenamiento seguro.</p>
