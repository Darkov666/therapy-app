    ...
    <AdminLayout title="Gestión de Productos">
        <div class="mb-6 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0 text-secondary-900 dark:text-white">
            ...
            <div class="flex space-x-4">
                <Link :href="route('admin.products.create')" class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700 transition">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-secondary-200 dark:divide-secondary-700">
                    <thead class="bg-primary-50 dark:bg-secondary-800">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-secondary-500 uppercase tracking-wider">Portada</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-secondary-500 uppercase tracking-wider">Producto</th>
                    ...
                    <tbody class="divide-y divide-secondary-200 dark:divide-secondary-700 bg-white dark:bg-secondary-900">
                        <tr v-for="product in products.data" :key="product.id" class="hover:bg-primary-50 dark:hover:bg-secondary-800 transition">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex-shrink-0 h-16 w-12">
                                    <img v-if="product.cover_path" :src="product.cover_url" class="h-16 w-12 object-cover rounded-md shadow-sm" alt="Cover">
                                    <div v-else class="h-16 w-12 bg-gray-200 dark:bg-gray-700 rounded-md flex items-center justify-center text-gray-400">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-secondary-900 dark:text-white">{{ product.title }}</div>
                            </td>
                             <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-xs text-secondary-500">{{ product.user ? product.user.name : 'Unknown' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-secondary-500 mt-1 dark:text-secondary-400">
                                <div>{{ formatCurrency(product.price_mxn, 'MXN') }}</div>
                                <div class="text-xs text-gray-400">{{ formatCurrency(product.price_usd, 'USD') }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                <Link :href="route('admin.products.edit', product.id)" class="text-yellow-600 hover:text-yellow-900" title="Editar">
                                    <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </Link>
                                <button @click="deleteProduct(product)" class="text-red-600 hover:text-red-900 focus:outline-none" title="Eliminar">
                                    <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                </button>
                            </td>
                        </tr>
                        <tr v-if="products.data.length === 0">
                            <td colspan="7" class="px-6 py-4 text-center text-sm text-secondary-500">
