...
const menuItems = [
    ...
    { name: 'Usuarios', route: 'admin.users.index', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
   // { name: 'Citas', route: 'admin.appointments.index', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
        ...
        <aside :class="{'translate-x-0': showingSidebar, '-translate-x-full': !showingSidebar}" class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-secondary-900 border-r border-primary-100 dark:border-secondary-800 transition-transform duration-300 md:relative md:translate-x-0">
            ...
            <nav class="mt-6 px-4 space-y-2">
                <Link v-for="item in menuItems" :key="item.name" :href="route(item.route)" 
                    :class="{'bg-primary-50 text-primary-600 dark:bg-secondary-800 dark:text-primary-400': route().current(item.route), 'text-secondary-600 hover:bg-primary-50 hover:text-primary-600 dark:text-secondary-400 dark:hover:bg-secondary-800 dark:hover:text-white': !route().current(item.route)}"
